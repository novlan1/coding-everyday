...
BEM 是块（block）、元素（element）、修饰符（modifier）的简写，由Yandex团队提出的一种前端CSS命名方法论。
...
3. 修饰符名称可以由拉丁字母、数字、破折号和下划线组成。CSS 类由块或元素的名称加上两个破折号组成：`.block--mod or .block__elem--mod`和`.block--color-black with .block--color-red`。复杂修饰符中的空格由破折号代替。
