...
date                      # 显示日期
date +"%Y_%m_%d_%H_%M_%S" # 日期格式如：2022_07_04_09_50_05
cal                       # 显示日历
...
uname                     # 显示系统版本号
uname -a                  # 查看全部机器信息，如 Linux VM-155-199-centos 3.10.107-1-tlinux2_kvm_guest-0049 #1 SMP Tue Jul 30 23:46:29 CST 2019 x86_64 x86_64 x86_64 GNU/Linux
uname -r                  # 查看内核的版本，如 3.10.107-1-tlinux2_kvm_guest-0049
uname -m                  # 查看CPU架构，如 x86_64
cat /etc/issue            # 查看操作系统的版本，如 Debian GNU/Linux 9 \n \l
...
# 小技巧
${path##*/}               # 获取文件名 (相当于basename命令的功能)
${path%/*}                # 获取目录名 (相当于dirname命令的功能)
${path##*.}               # 获取后缀名
...
str1 < str2               # 字符串小于，如 [ "$x" \< "$y" ] && echo yes，或 [[ "${myvar}" < "abc" ]]
str2 > str2               # 字符串大于，注意 < 或 > 是字面量，输入时要加反斜杆，如果是在[ ]中使用<和>，需要将它们写成\<和\，在[[]]中不需要

-n str1                   # 判断字符串不为空（长度大于零），比如 [ -n "${myvar}" ]
-z str1                   # 判断字符串为空（长度等于零），比如 [ -z "${myvar}" ]
