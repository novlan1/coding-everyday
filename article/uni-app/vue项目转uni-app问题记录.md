...
const uniSystemInfoMixin = {
  ...
  methods: {
    onSetFontSize() {
      const screenNumber = 7.5;	// 宽度 375 时（iphone6），rootFontSize为50，则一份为 375/50=7.5
      const that = this ;
...
因为这会导致所有js都会打包到珠宝店的common/main.js中
...
## 一个组件被import两次，对应的js就会编译错误

啥时候一个组件会被 import 两次呢，动态加载，比如`() => import('./xxx.comp')`

## path-to-regexp库使用时，keys不能是const
...
- https://ask.dcloud.net.cn/question/97718

## Vue TypeError: Right-hand side of 'instanceof' is not an object

Vue项目报上面这个错误，一般是props类型校验问题。这个搜索引擎一搜就能知道。


## uni-app传递对象props，对象中的function会被丢掉
...
<view value="val" bind:change="__e" bind:input="__e"
...
uniapp 中 vant 的 change 事件暴露参数为 event，需适配


```ts
this.curTab = e.target.name;
```